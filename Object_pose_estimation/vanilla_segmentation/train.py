...
if __name__ == '__main__':
    ...
    # dataset = SegDataset(opt.dataset_root, '/data2/leo/DenseFusion/data/train_data_list.txt', True)
    dataset = SegDataset(opt.dataset_root, 'train', True)
    dataloader = torch.utils.data.DataLoader(dataset, batch_size= opt.batch_size, shuffle=True, num_workers=int(opt.workers))
    # test_dataset = SegDataset(opt.dataset_root, '/data2/leo/DenseFusion/data/test_data_list.txt', True)
    test_dataset = SegDataset(opt.dataset_root, 'test', True)
