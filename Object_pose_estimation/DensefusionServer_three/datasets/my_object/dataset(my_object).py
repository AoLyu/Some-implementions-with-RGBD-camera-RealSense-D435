...
class PoseDataset(data.Dataset):
    def __init__(self, mode, num, add_noise, root, noise_trans, refine):
        ...
        for item in self.objlist:
        	if self.mode == 'train':
        		...
        		meta_file = open('/data2/leo/bop/bop_renderer/samples/newObject/{:02d}/gt.yml'.format(item), 'r')
        		meta = yaml.load(meta_file)
        		for image_id in range(9000):
        			self.list_rgb.append('/data2/leo/bop/bop_renderer/samples/newObject/{:02d}/rgb/{:04d}.png'.format(item,image_id))
        			self.list_depth.append('/data2/leo/bop/bop_renderer/samples/newObject/{:02d}/depth/{:04d}.png'.format(item,image_id))
        			self.list_label.append('/data2/leo/bop/bop_renderer/samples/newObject/{:02d}/label/{:04d}.png'.format(item,image_id))
        			self.list_meta.append(meta[image_id][0])
        			self.list_obj.append(item)

        	else:
        		meta_file = open('/data2/leo/bop/bop_renderer/samples/newObject/{:02d}/gt.yml'.format(item), 'r')
        		meta = yaml.load(meta_file)
        		for image_id in range(9000,10000):
        			self.list_rgb.append('/data2/leo/bop/bop_renderer/samples/newObject/{:02d}/rgb/{:04d}.png'.format(item,image_id))
        			self.list_depth.append('/data2/leo/bop/bop_renderer/samples/newObject/{:02d}/depth/{:04d}.png'.format(item,image_id))
        			self.list_label.append('/data2/leo/bop/bop_renderer/samples/newObject/{:02d}/label/{:04d}.png'.format(item,image_id))
        			self.list_meta.append(meta[image_id][0])
        			self.list_obj.append(item)
        	self.pt[item] = ply_vtx('/data2/leo/my_object/models/obj_{:02d}.ply'.format(item))
    ...
    def __getitem__(self, index):
        ...
        for m in range(ys.min(),ys.max()+1):
        	for n in range(xs.min(),xs.max()+1):
        		if depth[m,n] > 0:
        			background[m,n,:3] = img[m,n,:3]

        img = Image.fromarray(background.astype('uint8')).convert('RGB')
